...
def pixelate_gray(gray: np.ndarray, pixel_size: int) -> np.ndarray:
    ...
    # Downscale (can be linear), then upscale with NEAREST for crisp blocks
...
def metalwork_pixel_filter(img_bgr, pixel_size, low_thresh, high_thresh):
    """
    Canny edges -> thicken -> binarize -> PIXELATE to big squares.
    ...
    # Ensure clean 0/255
    binary = np.where(edges > 0, 255, 0).astype(np.uint8)

    # THIS is the key: pixelate with big visible squares
    pixelated = pixelate_gray(binary, pixel_size)
    return pixelated
...
if uploaded:
    ...
    # Download (REAL PNG bytes, not .tobytes())
    buf = io.BytesIO()
